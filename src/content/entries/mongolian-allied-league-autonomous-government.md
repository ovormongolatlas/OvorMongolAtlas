...
It was around this time — with the Japanese-run "Joint Committee of Mongolian Territories," formed on November 22, 1937, unifying this government with puppet regimes in Southern Chahar and Northern Shanxi — that the name "Mengjiang" entered common use for the combined territory (Jagchid). This is presently a single-source (Jagchid) account of the naming event itself, though it is not entirely uncorroborated: the [Mengjiang](/entry/mengjiang) entry's independent Tuo-sourced chronology records a "Mengjiang Federation Committee" forming at [Kalgan](/entry/kalgan) on the same date to unite the same three regimes — very likely the same body under a different translation of its name, though no source used in this atlas states that equivalence explicitly, so it is offered here as a probable but unconfirmed cross-corroboration rather than an established fact.
